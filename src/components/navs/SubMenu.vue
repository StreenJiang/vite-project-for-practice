...
const showToggleBar = computed(() => props.toggled ? "opacity-100 sxga:w-2 sm:w-1.5" : "opacity-0 w-0");
</script>
...
<template>
    <router-link :to="menu.routeTo" class="m-2 rounded-xl h-12 w-52 hover:cursor-pointer transition-all duration-300 select-none flex">
        <div class="sxga:h-6 sm:h-4 bg-gray-800 rounded-full sxga:ml-3 sxga:mr-1 sm:ml-2.5 sm:mr-0.5 my-auto transition-all duration-300" :class="showToggleBar"></div>
        <component :is="IconSvg" class="icon duration-300 my-auto sxga:size-7 sxga:mx-3 sm:size-5 sm:mx-2" :class="iconColor"/>
        <p class="text-nowrap font-bold duration-300 my-auto flex-grow sxga:mx-3 sxga:ml-1 sxga:text-base sm:mx-2.5 sm:ml-0.5 sm:text-sm" :class="textColor">{{ menu.name }}</p>
    </router-link>
</template>