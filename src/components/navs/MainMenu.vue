...
const menuClass = computed(() => props.isIconMode ? 'sxga:h-12 sxga:w-12 sm:h-8 sm:w-8' : 'sxga:h-24 sxga:w-24 sm:h-20 sm:w-20');
</script>
...
<template>
    <router-link :to="menu.routeTo" :class="[menuClass, 'hover:cursor-pointer transition-all duration-300 select-none flex flex-col items-center justify-center']">
        <component :is="IconSvg" :class="['icon duration-300', iconColor, props.isIconMode ? 'sxga:size-6 sm:size-5' : 'sxga:size-9 sm:size-7']"/>
        <p v-if="!isIconMode" class="pt-1 font-bold duration-300 text-center sxga:text-base sm:text-sm" :class="textColor">{{ menu.name }}</p>
    </router-link>
</template>